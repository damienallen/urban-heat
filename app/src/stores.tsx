...
export class AppStore {
	...
	get featureProperties() {
		return this.urbanExtents?.features.map(
			(feat: MapGeoJSONFeature) => feat.properties,
		);
	}
...
export class ContoursStore {
	...
	get thresholds() {
		if (this.stats) {
			...
			const uniqueThresholds = new Set(
				linspace(low, high, this.stats.st_dev / 2).map((val: number) =>
					Math.round(val),
				),
			);
	...
	featureFromPath = (path: string) => {
		...
		const feature = this.root.app.features.find(
			(feat: MapGeoJSONFeature) =>
				slugify(feat.properties.URAU_NAME) === cityName,
		);
		if (feature) {
	...
	initUrau = async () => {
		...
		const latestYear = this.annualData.reduce((max, s) =>
